...
import static com.projecki.fusion.util.BitMath.mask;
import static com.projecki.fusion.util.BitMath.unpackX;
import static com.projecki.fusion.util.BitMath.unpackY;
import static com.projecki.fusion.util.BitMath.unpackZ;
...
public record BlockVector3(int x, int y, int z) {
    ...
    public static boolean isLongPackable(BlockVector3 location) {
        return isHorizontallyInBounds(location.x())
                ...
                && 0 <= location.y() && location.y() <= WORLD_Y_MAX;
    }
    ...
    public BlockVector3 getMinimum(BlockVector3 v2) {
        int minX = Math.min(x, v2.x);
        int minY = Math.min(y, v2.y);
        int minZ = Math.min(z, v2.z);
    ...
    public BlockVector3 getMaximum(BlockVector3 v2) {
        int maxX = Math.max(x, v2.x);
        int maxY = Math.max(y, v2.y);
        int maxZ = Math.max(z, v2.z);
     ...
     * <p>
     * This is equivalent to {@code vector.equals(IntVector3.at(x, y, z))}
     * only without creating a new IntVector3 object.
     *
