...
public record Vector3(double x, double y, double z) {
    ...
    public static final Vector3 UNIT_NEGATIVE_X = new Vector3(-1, 0, 0);
    public static final Vector3 UNIT_NEGATIVE_Y = new Vector3(0, -1, 0);
    public static final Vector3 UNIT_NEGATIVE_Z = new Vector3(0, 0, -1);
     ...
     * <ul>
     *     <li>X coordinate for {@code 0}</li>
     *     <li>Y coordinate for {@code 1}</li>
     *     <li>Z coordinate for {@code 2}</li>
     * </ul>
     *
    ...
    public double get(int index) {
        return switch (index) {
            ...
            case 1 -> y;
            case 2 -> z;
            default -> throw new IllegalArgumentException("invalid index (not 0-2): " + index);
        };
    }
    ...
    public Vector3 clampY(int min, int max) {
        ...
        return y < min ? Vector3.at(x, min, z) : y > max ? Vector3.at(x, max, z) : this;
    }
    ...
    public Vector3 getMinimum(Vector3 v2) {
        double minX = Math.min(x, v2.x);
        double minY = Math.min(y, v2.y);
        double minZ = Math.min(z, v2.z);
        return this.equals(minX, minY, minZ) ? this :
                v2.equals(minX, minY, minZ) ? v2 :
    ...
    public Vector3 getMaximum(Vector3 v2) {
        double maxX = Math.max(x, v2.x);
        double maxY = Math.max(y, v2.y);
        double maxZ = Math.max(z, v2.z);
        return this.equals(maxX, maxY, maxZ) ? this :
                v2.equals(maxX, maxY, maxZ) ? v2 :
                        new Vector3(maxX, maxY, maxZ);
    }

    /**
     * Create a new {@code IntVector} using the given components.
     *
     * @param x the X coordinate
     * @param y the Y coordinate
     * @param z the Z coordinate
     * @return a new {@code IntVector}
     */
    ...
    /**
     * Create a new {@code IntVector} from this vector.
     *
     * @return a new {@code IntVector}
     */
    ...
    /**
     * Compare the given coordinates to the internal coordinates
     * of this vector for equality.
     * <p>
     * This is equivalent to {@code vector.equals(Vector3.at(x, y, z))}
     * only without creating a new Vector3 object.
     *
     * @param x The X coordinate to compare to {@link #x()}.
     * @param y The Y coordinate to compare to {@link #y()}.
     * @param z The Z coordinate to compare to {@link #z()}.
     * @return If the coordinates are equal to their counterpart
     *         within this vector.
     */
